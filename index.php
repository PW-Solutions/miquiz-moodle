...
foreach($miquizzes as $row) {
    array_push($quiz_table_body, array(
        ...
        "id" => $row['id'],
        "name" => $row['name'],
        "assesstimestart" => gmdate("d.m.Y, H:i", $row['assesstimestart']),
        "assesstimefinish" => gmdate("d.m.Y, H:i", $row['assesstimefinish']),
...
$PAGE->requires->js_amd_inline('$(document).ready(function(){$("#datatable").DataTable();});');
